...
- [Development guidelines](#development-guidelines)
    - [General SafeMine Core](#general-safemine-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
SafeMine Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/safeminepay/safemine/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_safemine
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_safemine --log_level=test_suite
$ valgrind -v --leak-check=full src/safemined -printtoconsole
```
...
# A coverage report will now be accessible at `./test_safemine.coverage/index.html`.
```

**Sanitizers**

SafeMine Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 38458.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 38457 for connections and services them.
...
However, in open source software such as SafeMine Core, where everyone uses
...
pay attention to for reviewers of SafeMine Core code.

General SafeMine Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: SafeMine Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against SafeMine Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `safemine-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `safemine-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of SafeMine Core, the wallet was always
